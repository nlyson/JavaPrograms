...
class Decode { 
 ...
 public static String decode(String r) {
    ...
    // Get code and word
    int code = Integer.parseInt(r.replaceAll("[A-Za-z]", ""));
    String word = r.replaceAll("\\d", "");
  
    
    // Map corresponding character values
    // Impossible to decode if duplicate keys are present
    Map<Character, Character> map = new HashMap<Character, Character>();
    
    for(int i=0; i<26; i++)
    {
      if(map.containsKey((char)(i*code%26+'a')))
        return "Impossible to decode";
      map.put((char)(i*code%26+'a'), (char)(i+'a')  );
    }
    
  
    // Decode the result and put in a String
    String output = "";
    
    for(String c : word.split(""))
      output += map.get(c.charAt(0)) + "";
